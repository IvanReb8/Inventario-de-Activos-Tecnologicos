...
import gob.imss.mx.products.repositories.TechnologyAssetRepository;
...
public class AssetServiceImpl implements AssetService {
    ...
    public TechnologyAsset registerAsset(AssetRequestDTO dto) {
       if (assetRepository.existsBySerialNumber(dto.getSerialNumber())) {
            throw new IllegalArgumentException("El número de serie ya se encuentra registrado.");
        }
    ...
    public TechnologyAsset updateAsset(String id, AssetRequestDTO dto) {
        ...
        if (assetRepository.existsBySerialNumberAndIdTechnicalNot(dto.getSerialNumber(), id)) {
            throw new IllegalArgumentException("El número de serie ya pertenece a otro activo.");
        }

        if ("DISPOSED".equalsIgnoreCase(asset.getStatus()) && !"DISPOSED".equalsIgnoreCase(dto.getStatus())) {
            throw new IllegalStateException("Un activo con estado Baja no podrá regresar a un estado operativo posterior.");
        }
    ...
    public ZipResponseDTO generateZipReport(String serial, String model, Long catId, String status, BigDecimal costMin,
            BigDecimal costMax, String requesterUser) throws IOException {
       List<TechnologyAsset> assets = assetRepository.listForReport(serial, model, catId, status, costMin, costMax);
        ...
        return new ZipResponseDTO(200, "Reporte generado correctamente", "inventario.zip", base64Zip);
    }
